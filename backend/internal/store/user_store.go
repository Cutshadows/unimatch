...
func (pg *PostgresUserStore) CreateUser(user *User) error {
	...
	err := pg.db.QueryRow(query, user.Username, user.Email, user.Bio).Scan(&user.ID, &user.CreatedAt, &user.UpdatedAt)
...
func (pg *PostgresUserStore) GetUserToken(scope, plaintextPassword string) (*User, error) {
	...
	query := `
	SELECT u.id, u.username, u.email, u.password_hash, u.bio, u.updated_at
	FROM users u
	...
	err := pg.db.QueryRow(query, tokenHash[:], scope, time.Now()).Scan(&user.ID, &user.Username, &user.Email, &user.PasswordHash.hash, &user.Bio, &user.UpdatedAt)
	if err == sql.ErrNoRows {
		return nil, nil
	}
	if err != nil {
